...
import { z } from 'zod';
import { User, Phone, Building2, GraduationCap, Lock, ShieldCheck, ChevronDown, CheckSquare } from 'lucide-react';
...
const staffSchema = z.object({
  ...
  assignedYears: z.array(z.string()).optional(),
});

type StaffFormData = z.infer<typeof staffSchema>;

export function StaffForm({ role }: { role: string }) {
  ...
  const onSubmit = async (data: StaffFormData) => {
    ...
    try {
      ...
      console.log('Frontend State - Staff Form:', finalData);
      if (profileImage) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-1.5 md:col-span-2">
          ...
          <div className="relative">
            ...
            <input type="text" readOnly value={role.charAt(0).toUpperCase() + role.slice(1)} 
        ...
        <div className="space-y-1.5 md:col-span-2">
          ...
          <div className="relative">
            ...
            <select {...register("department")} defaultValue="" className={`w-full pl-10 pr-10 py-3 bg-white border ${errors.department ? 'border-red-500' : 'border-gray-200 focus:border-[#CD0000] focus:ring-2 focus:ring-[#CD0000]/20'} rounded-xl outline-none transition-all text-gray-900 font-medium shadow-sm appearance-none`}>
          ...
          {errors.department && <p className="text-red-500 text-xs ml-1 font-medium">{errors.department.message}</p>}
        </div>
        ...
        {watchDepartment === 'Other' && (
          <div className="space-y-1.5 md:col-span-2 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Enter your department" {...register("departmentOther")}
        ...
        {/* Assigned Years (Multi-Select Optional) */}
        <div className="space-y-3 md:col-span-2">
          <label className="text-sm font-semibold text-gray-900 ml-1 block">Assigned Year(s) (Optional)</label>
          <div className="flex flex-wrap gap-4 px-1">
            {years.map(yr => (
              <label key={yr} className="flex items-center gap-2 cursor-pointer group">
                <input type="checkbox" value={yr} {...register("assignedYears")} className="hidden" />
                <div className="w-5 h-5 rounded border-2 border-gray-300 flex items-center justify-center group-hover:border-[#CD0000] transition-colors bg-white data-[state=checked]:bg-[#CD0000] data-[state=checked]:border-[#CD0000]">
                  {watch('assignedYears')?.includes(yr) && <CheckSquare className="w-4 h-4 text-[#CD0000]" />}
                </div>
                <span className="text-sm font-medium text-gray-700 select-none">Year {yr}</span>
              </label>
            ))}
          </div>
        </div>
